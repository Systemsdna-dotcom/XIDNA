...
/***********************************Custom Properties and Default Values****************************/
var CustomProperties = {
    fontColor: { DisplayName: "Font Color", IsForText: true, DefaultValue: "#ffffff" },
    fontStyle: { DisplayName: "Font Style", IsForText: true, DefaultValue: "normal" },
    fontSize: { DisplayName: "Font Size", IsForText: true, DefaultValue: 10 },
    fontWeight: { DisplayName: "Font Weight", IsForText: true, DefaultValue: "normal" },
    fontFamily: { DisplayName: "Font Family", IsForText: true, DefaultValue: "Lato, Helvetica, Arial, sans-serif" },
    text: { DisplayName: "Text", IsForText: true, DefaultValue: "Text" },
    textAlign: { DisplayName: "Text Align", IsForText: true, DefaultValue: "center" },
    font: { DisplayName: "Font", IsForText: true, DefaultValue: "normal normal 10pt Lato, Helvetica, Arial, sans-serif" },
    fontTransform: { DisplayName: "Font Transform", IsForText: true, DefaultValue: "Initial" },
    background: { DisplayName: "Background", IsForText: true, DefaultValue: "transparent" },

    IsHasText: { DisplayName: "Is Has Text", IsForText: false },
    color: { DisplayName: "Color", IsForText: false },
    fill: { DisplayName: "Fill", IsForText: false },
    size: { DisplayName: "Size", IsForText: false },
    isTooltip: { DisplayName: "Is Tooltip", IsForText: false },
    stroke: { DisplayName: "Stroke", IsForText: false },
    strokeWidth: { DisplayName: "Stroke Width", IsForText: false },
    category: { DisplayName: "Category", IsForText: false },
    key: { DisplayName: "Key", IsForText: false },
    LinkType: { DisplayName: "Link Type", IsForText: false, Default: "Normal" },
    IsLinkable: { DisplayName: "Is Linkable", IsForText: false, Default: true },
...
Inspector.prototype.inspectObject = function (object) {
    ...
    if (!inspectedObjects || !this.multipleSelection) {
        ...
        /*******************************Modified**********************/
        if (inspectedObject.data && inspectedObject.data.hasOwnProperty("category")) {
            document.getElementById("infoDraggableHandle").innerHTML = "Node Information";
        }
        else
            document.getElementById("infoDraggableHandle").innerHTML = "Link Information";

        if (this.inspectedObject.data.hasOwnProperty("IsHasText") && this.inspectedObject.data.IsHasText) {
            for (var property in CustomProperties) {
                if (CustomProperties[property].IsForText && this.inspectedObject.data[property] === undefined) {
                    this.inspectedObject.data[property] = CustomProperties[property].DefaultValue;
                }
            }
            this.inspectedObject.data.text = fontTransform(this.inspectedObject.data.text, this.inspectedObject.data.fontTransform);
        }
        /***************************************************************/
...
Inspector.prototype.buildPropertyRow = function (propertyName, propertyValue) {
    ...
    td1.textContent = CustomProperties[propertyName].DisplayName;/*****************Modified**********************/
    tr.appendChild(td1);
...
Inspector.prototype.updateAllProperties = function () {
    ...
    if (diagram.selection.count === 1 || !this.multipleSelection) { // single object update
        ...
        /**********************************code change for text******************************/
        var IsHasText = (inspectedProps.hasOwnProperty("IsHasText")) ? inspectedProps["IsHasText"].value : undefined;
        ...
        for (var name in inspectedProps) {
            ...
            var decProp = this.declaredProperties[name];
            if (!this.canEditProperty(name, decProp, this.inspectedObject) && name != "font") continue;
            ...
            if (CustomProperties[name].IsForText) {
                if (IsHasText == 'true') {
                    if (data.hasOwnProperty(name))
                        diagram.model.setDataProperty(data, name, value);
                    else
                        diagram.model.setDataProperty(data, name, CustomProperties[name].DefaultValue);
                    if (this.propertyModified !== null) this.propertyModified(name, value, this);
                }
                else {
                    if (data.hasOwnProperty(name)) {
                        if (this.propertyModified !== null) this.propertyModified(name, value, this, true);
                        //delete data[name];
                    }
                }
            ...
            else if (!CustomProperties[name].IsForText) {
                diagram.model.setDataProperty(data, name, value);
